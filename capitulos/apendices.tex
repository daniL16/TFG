...
%¿meter esto?
\chapter[Visualización de los puntos críticos.]{Visualización de los puntos críticos del gradiente sobre la esfera}\label{cod.vis}
\begin{lstlisting}[language=Python]
# uso: python3 gradient.py n k [anim]
...
def pol(self, t):
	c = ((self.n + self.k) * (self.n + self.k - 1))
		 / (2 * (2 * self.k - 1))
	d = (self.k + 0.5)
	return c * 
		gegenbauer(self.n - self.k, self.k - 0.5, t) 
	+ d * (1 - t * t) *
	gegenbauer(self.n - self.k - 2,self.k + 1.5, t)
...
def getPoints(self):
	...
	for i in range(0, 2 * self.k):
		phis.append(-math.pi / (2 * self.k) 
					+ (i * math.pi) / self.k)

	# soluciones del gegenbauer
	print("calculating")
	t = Symbol('t')
	solved = solve(self.pol(t),
	 		t,quintics=False,quartics=False,cubics=False)
...
def pintarEsfera(self):
	...
	theta, phi = np.linspace(0, 2 * np.pi, 50), 
...
def printPoints(self, color=(1, 0, 0.3)):
	...
	self.plt = mlab.points3d(xx, yy, zz, 
				scale_factor=0.05, color=color)
