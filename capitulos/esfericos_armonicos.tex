...
\begin{thm}\label{thm:1}Sean $\mathds{Y}^d \in \mathds{Y}^d_n$, $\xi\in\mathds{S}^{d-1}$. Entonces $\mathds{Y}_n$ es invariante respecto a $\mathds{O}^d(\xi)$, si y sólo si, $\mathds{Y}_n(\eta)=\mathds{Y}_n(\xi)\mathds{P}_{n,d}(\xi.\eta)  \quad \forall \eta\in\mathds{S}^{d-1}$
\end{thm}
\begin{proof}
$(\Rightarrow)$ Dado que $\xi$ es un vector unitario podemos encontrar $A_1\in \mathds{O}^d$ tal que $\xi = A_1e_d$. Sea $\overset{~}{Y_n(\eta)} = Y_n(A_1\eta),\eta\in\sphere$, que es invariante respecto a $\orto(e_d)$. De la definición de armónico de Legendre sabemos que $r^n\overset{~}{Y_n(\eta)} = c_1L_{n_d}(r^n\eta), r\ge0, \eta\in\sphere$ con $c_1$ una constante.\\
...
\end{thm}
\begin{proof}
Veamos que si $P_{n,d}:\mathds{V} \to \spharm$ es una biyección entonces\\ $\mathds{V}=\spharm$.
\medskip
...
Como $\tilde{P}_{n,m}:\mathds{Y}_{n,m}^{d-1} \to \mathds{Y_{m}^{d}}$ es una biyección entonces $$dim \mathds{Y}_{n,m}^{d} = dim \mathds{Y}_{m}^{d-1} = N_{m,d-1}$$
...
Por otro lado tenemos que (\hyperref[]{\ref{note:fun_leg}})
$$ 
